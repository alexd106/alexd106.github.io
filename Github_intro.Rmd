...
Just a note of caution. In this tutorial we'll be using RStudio to interface with Git as it gives you a nice friendly graphical user interface. However, the downside to using RStudio with Git is that RStudio only provides pretty basic Git functionality (more than enough to get you going though). If you really want to benefit from using Git's power you will need to learn some Git commands using the command line and that is a whole different tutorial! See some of the [links](#resources) below if you do want to dip your toes into the wonderful world of Git. 
...
Before you commit your changes notice the status of `first_doc.pdf` has changed from untracked to added (A). You can also view the changes you have made to the `first_doc.Rmd` by clicking on the file name in the top left pane which will then provide you with a useful summary of the changes in the bottom pane. Lines that have been deleted are highlighted in red and lines that have been added are highlighted in green (note that from Git's point of view, a modification to a line is actually two operations: the removal of the original line followed by the creation of a new line). Once you're happy, commit these changes by writing a suitable commit message and click on the 'Commit' button.

...
RStudio will now **clone** your remote repository to your local computer will create a new version controlled directory with the same name as your repository. This directory will contain two new files; `first_repo.Rproj` (or whatever you called your repository) and `README.md`. That's it for option 1, you now have a remote GitHub repository set up and this is linked to your local repository managed by RStudio. Any changes you make to files in this directory will be version controlled by Git.
...
OK, before we go on to create a repository on GitHub we need to do one more thing - we need to place our `second_repo.Rproj` and `.gitignore`files under version control. Unfortunately we haven't covered this in detail yet so if you want to use this method just follow the next few instructions (blindly!) and we'll revisit them in the [Using Git](#use_git) section of this tutorial.  
...
OK, lets go through an example to help clarify this workfow. 

...
R markdown is a simple and easy to use plain text language used to combine your R code, results from your data analysis (plots and tables) and written commentary into a single nicely formatted document (like a report, publication, thesis chapter and even a web page like this one).  
...
Note, when formatting text in an R markdown document the meaning of the `#` symbol is different than in a standard R script (which is used to included a comment - remember?!). You can, however, use a `#` symbol to comment code inside a [code chunk](#code_chunks) as usual (more about this in a bit).
